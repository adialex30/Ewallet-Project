...
import { syncTopupStatus } from '../api/topupApi'
...
export default function TransactionDetailModal({ open, transactionId, onClose, onSynced }) {
    ...
    const handleSync = async () => {
        setSyncing(true)
        try {
            const result = await syncTopupStatus(tx.orderId)

            if (result.status === 'SUCCESS') {
                toast.success('Status terkonfirmasi settlement - saldo sudah diperbarui.')
            } else if (result.status === 'FAILED') {
                toast.error('Pembayaran ini ditolak/dibatalkan di Midtrans.')
            } else {
                toast.info('Masih PENDING di Midtrans, coba cek lagi beberapa saat.')
            }
            ...
            toast.error(err.message || 'Gagal mengecek status ke Midtrans.')
        } finally {
            setSyncing(false)
        }
    }
            ...
            <div className="w-full max-w-sm rounded-2xl bg-white p-6 shadow-xl">
                ...
                {loading ? (
                    ...
                    <div className="mt-4">
                        ...
                        {tx.type === 'DEPOSIT' && tx.status === 'PENDING' && tx.orderId && (
                            <button
                                type="button"
                                onClick={handleSync}
                                disabled={syncing}
                                className="mt-4 flex w-full items-center justify-center gap-2 rounded-lg bg-primary py-2.5 text-sm font-semibold text-white transition hover:bg-primary-dark disabled:cursor-not-allowed disabled:opacity-60"
                            >
                                {syncing && <Spinner className="h-4 w-4" />}
                                {syncing ? 'Mengecek ke Midtrans...' : 'Cek Status ke Midtrans'}
                            </button>
                        )}
                    </div>
                ) : null}
