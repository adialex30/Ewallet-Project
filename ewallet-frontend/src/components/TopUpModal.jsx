...
import { createTopup, syncTopupStatus } from '../api/topupApi'
...
async function pollUntilSettled(orderId, { attempts = 10, intervalMs = 3000 } = {}) {
  for (let i = 0; i < attempts; i++) {
    await new Promise((resolve) => setTimeout(resolve, intervalMs))
    try {
      const result = await syncTopupStatus(orderId)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (outcome.status === 'success' || outcome.status === 'pending') {
        ...
        pollUntilSettled(orderId).then((result) => {
